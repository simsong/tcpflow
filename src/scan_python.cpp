// -*- mode: C++; c-basic-offset: 4; indent-tabs-mode: nil; tab-width: 8; c-file-style: "linux" -*-
// vi:ft=cpp ts=8 et sw=4 sts=4 tw=80
/**
 *
 * scan_python:
 * Use external python script
 */

#include "config.h"
#include "bulk_extractor_i.h"
#include "dfxml/src/hash_t.h"
#include "tcpflow.h"

#include <iostream>
#include <sys/types.h>

#include <python2.7/Python.h> // Get header: install package "python-devel"

// Import the global variable that holds our commandline argument for -P
extern std::string pyPluginArg;

extern "C" void scan_python(const class scanner_params& sp,
                            const recursion_control_block& rcb)
{
    if (sp.sp_version != scanner_params::CURRENT_SP_VERSION) {
        std::cerr << "scan_python requires sp version "
                  << scanner_params::CURRENT_SP_VERSION << "; "
                  << "got version " << sp.sp_version << "\n";
        exit(1);
    }

    if (sp.phase == scanner_params::PHASE_STARTUP) {
        sp.info->name = "python";
        sp.info->flags = scanner_info::SCANNER_DISABLED;
        return; /* No feature files created */
    }

#ifdef HAVE_EVP_GET_DIGESTBYNAME
    if (sp.phase == scanner_params::PHASE_SCAN) {
        // Find delimeter "::" in command line argument
        size_t delimeterIndex = pyPluginArg.rfind("::");
        if (delimeterIndex == std::string::npos) {
            std::cerr <<
              "Argument to option '-P' does not respect format 'scriptname::function'.\n";
            exit(1);
        }

        // Split the argument
        std::string pluginName = pyPluginArg.substr(0, delimeterIndex);
        std::string functionName = pyPluginArg.substr(delimeterIndex + 2);

        // Spawn python interpreter and define python objects
        Py_Initialize();

        // Cast packet buffer contents into a string and then create pyObject from
        // string
        std::string data(reinterpret_cast<char const*>(sp.sbuf.buf));
        PyObject *pData = PyString_FromString(data.c_str());

        // Add the plugin directory to the local system path
        PyRun_SimpleString(
            "import sys, os\n"
            "workingDir = os.getcwd() + '/python/plugins'\n"
            "sys.path.append(workingDir)\n");

        // Create pyObject from plugin file name (string)
        PyObject *pName = PyString_FromString(pluginName.c_str());

        // Import plugin file in python interpreter; if an import error occurs,
        // return to display the error
        PyObject *pModule = PyImport_Import(pName);
        if (pModule == NULL)
            return;

        // Identify plugin function to be used; if an assignment error occurs,
        // return to display the error
        PyObject *pFunc = PyObject_GetAttrString(pModule, functionName.c_str());
        if (pFunc == NULL)
            return;

        // Compose python argument in the form of a tuple and pass the argument to
        // the chosen function; if the function does not return anything or
        // encounters an error, return to exit and/or display the error
        PyObject *pArgs = PyTuple_New(1);
        PyTuple_SetItem(pArgs, 0, pData);
        PyObject *pResult = PyObject_CallObject(pFunc, pArgs);
        if (pResult != NULL) {
            // If xml-reporting is enabled, insert the string the function returned
            // into the report
            if (sp.sxml) {
                (*sp.sxml) << "<plugindata>\n"
                              "      " << PyString_AsString(pResult)
                           << "\n"
                              "      </plugindata>";
            }
        }
        // Terminate the python interpreter and exit
        Py_Finalize();
        return;
    }
#endif
}
